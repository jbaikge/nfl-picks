...
func (s *Store) SavePick(u *User, p *Pick) (err error) {
	query := `INSERT OR REPLACE INTO picks
		(user_id, game_id, pick_value)
		VALUES
		(?,       ?,       ?         )
	`
	_, err = s.db.Exec(query, p.UserId, p.GameId, p.Value())
	return
}
...
func (s *Store) SaveOdds(o *Odds) (err error) {
	query := `INSERT OR REPLACE INTO odds
		(game_id, odds_spread, odds_over_under)
		VALUES
		(?,       ?,           ?              )
	`
	_, err = s.db.Exec(query, o.GameId, o.Spread, o.OverUnder)
	return
}
...
func (s *Store) Setup() (err error) {
	queries := []string{
		...
		`CREATE TABLE IF NOT EXISTS odds (
			game_id         TEXT NOT NULL REFERENCES games (game_id) UNIQUE,
			odds_spread     FLOAT(5,2) NOT NULL DEFAULT 0.0,
			odds_over_under FLOAT(5,2) NOT NULL DEFAULT 0.0
		)`,
		`INSERT OR IGNORE INTO odds VALUES
			("GBvSEA@20140904",   -5.5, 46),
			("NOvATL@20140907",      3, 51.5),
			("MINvSTL@20140907",    -4, 44),
			("CLEvPIT@20140907",  -6.5, 41.5),
			("JACvPHI@20140907", -10.5, 52.5),
			("OAKvNYJ@20140907",    -5, 40),
			("CINvBAL@20140907",  -1.5, 43),
			("BUFvCHI@20140907",    -7, 47.5),
			("WASvHOU@20140907",  -2.5, 45.5),
			("TENvKC@20140907",     -4, 43.5),
			("NEvMIA@20140907",      5, 47.5),
			("CARvTB@20140907",     -2, 39),
			("SFvDAL@20140907",      5, 51),
			("INDvDEN@20140907",  -7.5, 55.5),
			("NYGvDET@20140908",  -5.5, 47),
			("SDvARI@20140908",     -3, 45),
			("PITvBAL@20140911",  -2.5, 44),
			("MIAvBUF@20140914",     1, 43),
			("JACvWAS@20140914",  -5.5, 43),
			("DALvTEN@20140914",  -3.5, 49),
			("ARIvNYG@20140914",     2, 43),
			("NEvMIN@20140914",    3.5, 49),
			("NOvCLE@20140914",    6.5, 48),
			("ATLvCIN@20140914",    -5, 48),
			("DETvCAR@20140914",  -2.5, 44),
			("STLvTB@20140914",   -5.5, 37),
			("SEAvSD@20140914",      6, 45),
			("HOUvOAK@20140914",     3, 39),
			("NYJvGB@20140914",   -8.5, 46),
			("KCvDEN@20140914",  -13.5, 52),
			("CHIvSF@20140914",     -7, 48),
			("PHIvIND@20140915",    -3, 54),
			("TBvATL@20140918",   -6.5, 45),
			("SDvBUF@20140921",   -2.5, 44),
			("DALvSTL@20140921",     1, 45),
			("WASvPHI@20140921",  -6.5, 50),
			("HOUvNYG@20140921",   2.5, 41),
			("MINvNO@20140921",  -10.5, 50),
			("TENvCIN@20140921",    -7, 43),
			("BALvCLE@20140921",   1.5, 42),
			("GBvDET@20140921",     -2, 53),
			("INDvJAC@20140921",     7, 45),
			("OAKvNE@20140921",  -14.5, 47),
			("SFvARI@20140921",      3, 42),
			("DENvSEA@20140921",  -4.5, 49),
			("KCvMIA@20140921",   -4.5, 42),
			("PITvCAR@20140921",  -3.5, 42),
			("CHIvNYJ@20140922",  -2.5, 45),
			("NYGvWAS@20140925",    -4, 46.5),
			("GBvCHI@20140928",    1.5, 49.5),
			("BUFvHOU@20140928",    -3, 41),
			("TENvIND@20140928",  -7.5, 46),
			("CARvBAL@20140928",  -3.5, 41),
			("DETvNYJ@20140928",   1.5, 45),
			("TBvPIT@20140928",   -7.5, 44.5),
			("MIAvOAK@20140928",     4, 40.5),
			("JACvSD@20140928",  -13.5, 45),
			("ATLvMIN@20140928",     3, 47),
			("PHIvSF@20140928",     -5, 51),
			("NOvDAL@20140928",      3, 53.5),
			("NEvKC@20140929",     3.5, 46),
			("MINvGB@20141002",     -9, 48),
			("CHIvCAR@20141005",  -2.5, 46),
			("CLEvTEN@20141005",     1, 44),
			("STLvPHI@20141005",    -7, 48),
			("ATLvNYG@20141005",    -4, 50.5),
			("TBvNO@20141005",     -10, 48.5),
			("HOUvDAL@20141005",    -6, 46.5),
			("BUFvDET@20141005",    -7, 43.5),
			("BALvIND@20141005",  -3.5, 49),
			("PITvJAC@20141005",   6.5, 46.5),
			("ARIvDEN@20141005",    -7, 49),
			("KCvSF@20141005",    -6.5, 44.5),
			("NYJvSD@20141005",     -7, 44),
			("CINvNE@20141005",    1.5, 46),
			("SEAvWAS@20141006",   7.5, 46),
			("INDvHOU@20141009",     3, 47),
			("DENvNYJ@20141012",     9, 48),
			("PITvCLE@20141012",    -2, 47),
			("JACvTEN@20141012",    -6, 0),
			("GBvMIA@20141012",    3.5, 49.5),
			("DETvMIN@20141012",     2, 0),
			("CARvCIN@20141012",    -7, 43.5),
			("NEvBUF@20141012",      3, 0),
			("BALvTB@20141012",    3.5, 43.5),
			("SDvOAK@20141012",    7.5, 43.5),
			("CHIvATL@20141012",    -3, 54),
			("DALvSEA@20141012",    -8, 47.5),
			("WASvARI@20141012",  -3.5, 0),
			("NYGvPHI@20141012",  -2.5, 50.5),
			("SFvSTL@20141013",    3.5, 44),
			("NYJvNE@20141016",    -10, 45),
			("ATLvBAL@20141019",    -7, 49.5),
			("TENvWAS@20141019",  -5.5, 46.5),
			("SEAvSTL@20141019",     7, 43.5),
			("CLEvJAC@20141019",     6, 45),
			("CINvIND@20141019",  -3.5, 50),
			("MINvBUF@20141019",  -5.5, 42.5),
			("MIAvCHI@20141019",    -4, 49),
			("NOvDET@20141019",     -3, 48.5),
			("CARvGB@20141019",     -7, 49),
			("KCvSD@20141019",      -4, 45.5),
			("ARIvOAK@20141019",     4, 44),
			("NYGvDAL@20141019",  -6.5, 48.5),
			("SFvDEN@20141019",   -6.5, 51.5),
			("HOUvPIT@20141020",  -3.5, 45),
			("HOUvPIT@20141020",  -3.5, 45),
			("SDvDEN@20141023",     -8, 52.5),
			("DETvATL@20141026",     4, 47),
			("STLvKC@20141026",     -7, 44),
			("HOUvTEN@20141026",   2.5, 43),
			("MINvTB@20141026",     -3, 42),
			("SEAvCAR@20141026",   5.5, 45.5),
			("BALvCIN@20141026",     1, 46),
			("MIAvJAC@20141026",     6, 43.5),
			("CHIvNE@20141026",     -6, 51),
			("BUFvNYJ@20141026",    -3, 41),
			("PHIvARI@20141026",  -2.5, 48.5),
			("OAKvCLE@20141026",    -7, 43.5),
			("INDvPIT@20141026",   3.5, 49),
			("GBvNO@20141026",       0, 56.5),
			("WASvDAL@20141027",   -10, 50.5),
			("WASvDAL@20141027",   -10, 50.5),
			("NOvCAR@20141030",      3, 49.5),
			("TBvCLE@20141102",     -7, 44),
			("ARIvDAL@20141102",    -3, 45),
			("PHIvHOU@20141102",   2.5, 49),
			("NYJvKC@20141102",    -10, 42),
			("JACvCIN@20141102", -11.5, 43.5),
			("SDvMIA@20141102",     -1, 45),
			("WASvMIN@20141102",  -1.5, 45.5),
			("STLvSF@20141102",    -10, 44),
			("DENvNE@20141102",    3.5, 56),
			("OAKvSEA@20141102",   -15, 43.5),
			("BALvPIT@20141102",     1, 48.5),
			("INDvNYG@20141103",   3.5, 52),
			("CLEvCIN@20141106",  -6.5, 45.5),
			("KCvBUF@20141109",    2.5, 42),
			("MIAvDET@20141109",    -3, 44),
			("DALvJAC@20141109",     7, 45),
			("SFvNO@20141109",      -5, 49.5),
			("TENvBAL@20141109",   -10, 44),
			("PITvNYJ@20141109",   6.5, 45.5),
			("ATLvTB@20141109",      2, 46),
			("DENvOAK@20141109",  12.5, 50),
